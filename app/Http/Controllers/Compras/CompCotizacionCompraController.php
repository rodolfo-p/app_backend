...
class CompCotizacionCompraController extends Controller

{
    private $base_ruta_imagen;
    private $ruta_imagen_subir;
    public function __construct()
    {
        $this->base_ruta_imagen = public_path() . '/public/imagen';
        $this->ruta_imagen_subir = 'public/imagen/';
    }
    private function subir_imagen_articulo($imagen, $imagen_nombre)
    {
        try {

            $imagen_nombre_direccion = '/' . $imagen_nombre;
            $imagen->move($this->ruta_imagen_subir, $imagen_nombre_direccion);
            $ruta_imagen = $this->base_ruta_imagen . $imagen_nombre_direccion;
            $imagen_resize = Image::make($ruta_imagen);
            // $imagen_resize->resize(400, 400);
            $imagen_resize->save($ruta_imagen);
            return true;
    ...
    public function registrar_compra(Request $request)
    {
        ...
        $compra_data->comp_compra_tipo = 'CC';
        $compra_data->comp_compra_desc = $request->input('comp_compra_desc');
        $compra_data->comp_compra_serie_ref = $request->input('comp_compra_serie_ref');
        $compra_data->comp_compra_numero_venta_ref = $request->input('comp_compra_numero_venta_ref');
        $compra_data->save();
        $contador = 1;
        ...
        return response()->json(['success' => true,
            'data' => Pagination::paginatorFacturaPeru(Compra::select(
                ...
                ->where('comp_compra.comp_compra_tipo', 'CC')
    ...
    public function listar_compras(Request $request)
    {
        ...
        return response()->json(['success' => true,
            'data' => Pagination::paginatorFacturaPeru(Compra::select(
                ...
                'comp_compra.comp_compra_estado_pago',
                'comp_compra.comp_compra_serie',
                'comp_compra.comp_compra_serie_ref',
                'comp_compra.comp_compra_numero_venta_ref',
...
public function listar_compras_aprobados_administrador(Request $request){
    $condicional_tipo_comprobante = !is_null($request->input('comp_compra_tipo_comprobante_id')) ? '=' : '<>';
    $condicional_serie = !is_null($request->input('comp_compra_serie')) ? '=' : '<>';
    $condicional_numero = !is_null($request->input('comp_compra_numero_venta')) ? '=' : '<>';
    if (!is_null($request->input('comp_compra_fecha_desde')) &&
    ...
    return response()->json(['success' => true,
        'data' => Pagination::paginatorFacturaPeru(Compra::select(
            ...
            ->leftJoin('alm_almacen', 'comp_compra.comp_compra_almacen_id', 'alm_almacen.alm_almacen_id')
            ->where('comp_compra.comp_compra_tipo', 'CC')
            ->where('comp_compra.comp_compra_validacion_1', true)
            ->where('comp_compra.comp_compra_serie', $condicional_serie, is_null($request->input('comp_compra_serie')) ? null : $request->input('comp_compra_serie'))
            ->where('comp_compra.comp_compra_numero_venta', $condicional_numero, is_null($request->input('comp_compra_numero_venta')) ? null : $request->input('comp_compra_numero_venta'))
            ->where('comp_compra.comp_compra_tipo_comprobante_id', $condicional_tipo_comprobante, is_null($request->input('comp_compra_tipo_comprobante_id')) ? null : TipoComprobante::select('doc_tipo_comprobante_id')->where('doc_tipo_comprobante_codigo', $request->input('comp_compra_tipo_comprobante_id'))->first()->doc_tipo_comprobante_id)
            ->whereBetween('comp_compra.comp_compra_fecha', $condicional_fecha)
    ...
    public function listar_compras_aprobados_gerencia(Request $request){
        $condicional_tipo_comprobante = !is_null($request->input('comp_compra_tipo_comprobante_id')) ? '=' : '<>';
        $condicional_serie = !is_null($request->input('comp_compra_serie')) ? '=' : '<>';
        $condicional_numero = !is_null($request->input('comp_compra_numero_venta')) ? '=' : '<>';
        if (!is_null($request->input('comp_compra_fecha_desde')) &&
        ...
        return response()->json(['success' => true,
            'data' => Pagination::paginatorFacturaPeru(Compra::select(
                ...
                ->where('comp_compra.comp_compra_tipo', 'CC')
                ->where('comp_compra.comp_compra_validacion_1', true)
                ->where('comp_compra.comp_compra_validacion_2', true)
                ->where('comp_compra.comp_compra_serie', $condicional_serie, is_null($request->input('comp_compra_serie')) ? null : $request->input('comp_compra_serie'))
                ->where('comp_compra.comp_compra_numero_venta', $condicional_numero, is_null($request->input('comp_compra_numero_venta')) ? null : $request->input('comp_compra_numero_venta'))
                ->where('comp_compra.comp_compra_tipo_comprobante_id', $condicional_tipo_comprobante, is_null($request->input('comp_compra_tipo_comprobante_id')) ? null : TipoComprobante::select('doc_tipo_comprobante_id')->where('doc_tipo_comprobante_codigo', $request->input('comp_compra_tipo_comprobante_id'))->first()->doc_tipo_comprobante_id)
                ->whereBetween('comp_compra.comp_compra_fecha', $condicional_fecha)
    ...
    public function listar_compra($id)
    {
        ...
        if ($compra->comp_compra_imagen!= null){
            $compra->comp_compra_imagen = url('/') . '/' . $this->ruta_imagen_subir . $compra->comp_compra_imagen;
        }
    ...
    public function listar_compra_serie_numero_referencia(Request $request)
    {
        $compra = Compra::select('*')->where('comp_compra_serie', $request->input('comp_compra_serie_ref'))->where('comp_compra_numero_venta', $request->input('comp_compra_numero_venta_ref'))->first();
        $compra->proveedor = !is_null(Proveedor::find($compra->comp_compra_preveedor_id)) ? Proveedor::find($compra->comp_compra_preveedor_id)->alm_proveedor_razon_social : null;
        $compra->tipo_comprobante_codigo = TipoComprobante::find($compra->comp_compra_tipo_comprobante_id)->doc_tipo_comprobante_codigo;
        $compra->detalle = CompraDetalle::where('comp_compra_compra_id', $compra->comp_compra_id)
            ->orderBy('comp_compra_detalle_item')
            ->get();
        $pago = new  \stdClass();
        $pago->comp_pago_proveedores_tipo_pago = '02';
        $compra->pago_proveedores = !is_null(PagoProveedores::where('comp_pago_proveedores_id_compra', $compra->comp_compra_id)->first()) ? PagoProveedores::where('comp_pago_proveedores_id_compra', $id)->first() : $pago;
        return response()->json(['success' => true,
            'data' => $compra,
            'message' => 'Registro de compras'], 200);
    }
    ...
    public function subir_imagen(Request  $request){
        $compra = Compra::find($request->input('comp_compra_id'));
        if ($comp_compra_imagen = $request->file('comp_compra_imagen')) {
            $imagen_nombre = rand(100, 1000) . date('YmdHis') . "." . $comp_compra_imagen->getClientOriginalExtension();
             if ($this->subir_imagen_articulo($comp_compra_imagen, $imagen_nombre)){
                 $compra->comp_compra_imagen=$imagen_nombre;
                 $compra->save();
             };
    ...
    public function cambiar_estados_cotizacion(Request  $request, $id){
